<?php
/**
 * abc-sushi functions and definitions.
 *
 ...
 */
function abc_sushi_setup() {
	/*
	 * Make theme available for translation.
	 * Translations can be filed in the /languages/ directory.
	 * If you're building a theme based on abc-sushi, use a find and replace
	 * to change 'abc-sushi' to the name of your theme in all the template files.
	 */
	load_theme_textdomain( 'abc-sushi', get_template_directory() . '/languages' );
	...
	register_nav_menus( array(
		'primary' => esc_html__( 'Primary', 'abc-sushi' ),
	) );
	...
	add_theme_support( 'custom-background', apply_filters( 'abc_sushi_custom_background_args', array(
		'default-color' => 'ffffff',
		'default-image' => '',
	) ) );
}
endif;
add_action( 'after_setup_theme', 'abc_sushi_setup' );
 ...
 */
function abc_sushi_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'abc_sushi_content_width', 640 );
}
add_action( 'after_setup_theme', 'abc_sushi_content_width', 0 );
 ...
 */
function abc_sushi_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar', 'abc-sushi' ),
		'id'            => 'sidebar-1',
		'description'   => '',
...
add_action( 'widgets_init', 'abc_sushi_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function abc_sushi_scripts() {
	wp_enqueue_style( 'abc-sushi-style', get_stylesheet_uri() );

	wp_enqueue_script( 'abc-sushi-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );

	wp_enqueue_script( 'abc-sushi-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );
